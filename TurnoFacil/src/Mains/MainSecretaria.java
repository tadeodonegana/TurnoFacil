...
public class MainSecretaria {
	...
	private static AppSecretaria crearApp(Secretaria sec) { //m�todo privado para crear y retornar una instancia de AppSecretaria necesaria para el testeo
		//M�dicos
	...
	public static void main(String[] args) { //main de la Secretaria que permite utilizar los m�todos de la AppSecretaria a trav�s de un men� y con el mismo testear el c�digo
		System.out.println("*************** Turno Facil Secretaria ***************");
		//Secretaria
		Secretaria sec = new Secretaria("Patricia", "Suarez", "xxxx", "xxxx", 14758);
		AppSecretaria app = crearApp(sec); //crearApp recibe la Secretaria sec por par�metro para que luego se puedan invocar m�todos de la misma para el testeo
		int eleccion = 1;
						...
						for (Medico medico: medicos) {
							...
							if (jornadas.size() > 0) {
								...
								for (JornadaDiaria jornada: jornadas)
									System.out.println(jornada); //se imprimen las jornadas por cada m�dico para testear y saber si funciona correctamente el c�digo
							}
							else
								System.out.println("El medico no posee jornadas"); //si un m�dico no posee jornadas se avisa por pantalla para facilitar el testeo
						}
				}
						break;
				case 2: {
						...
						for (ParMedicoTurnos par: pares) {
							...
							for (Turno turno: turnos)
								System.out.println(turno); //se imprimen todos los turnos (sin separar por m�dico ya que al imprimir el turno ya se avisa a cu�l m�dico corresponde) para testear y saber si funciona correctamente el c�digo
						}	
				}
						break;
				case 3: {
