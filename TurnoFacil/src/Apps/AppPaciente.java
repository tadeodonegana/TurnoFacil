...
public class AppPaciente {
	...
	private FiltroTurno MenuFiltroTurno() {
		System.out.println("A continuacion podra indicar si desea filtrar los turnos por turno ma�ana(1), turno tarde(2), rango de fechas(3) o ningun criterio(4)");
		int eleccion = elegir();
		switch(eleccion) {
	...
	private int seleccionarMedico(ArrayList<Medico> medicos) {
		...
		while (true) {
			...
			if (eleccion >= 1 && eleccion <= medicos.size()) {
				...
				System.out.println("Dias y Horarios que trabaja el m�dico" + " " + medicos.get(eleccion-1).getNombreYApellido());
